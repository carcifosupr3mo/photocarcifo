...
| Cosa | Dove |
|---|---|
| Host virtuale | Proxmox, nodo `pve`, <IP host Proxmox> |
| Container | LXC 206 `<nome host container>`, <IP container applicazione>, Ubuntu, disco 64 GB |
| Archivio fotografie | Synology `magazzino`, <IP NAS Synology> |
...
Certificati Let's Encrypt per tutti e tre, rinnovo automatico
verificato.
...
## Infrastruttura
- Container Ubuntu Server 24.04 LTS — `<IP container applicazione>`
- Synology (hostname `magazzino`) — `<IP NAS Synology>`, cartella foto `/volume1/Foto`
...
## Accesso
- Sito pubblico: http://<IP container applicazione>
- Dashboard admin: http://<IP container applicazione>/admin
...
flowchart LR
    ...
    RO -.->|stesso volume| NAS[("Synology 'magazzino'<br/><IP NAS Synology><br/>/volume1/photocarcifo")]
...
- **Pagina di blocco `/bloccato` — jail web contro jail non-web** (aggiunto 06/09/2026): chi è bloccato dal *sito* non trova più la porta chiusa, ma una pagina che glielo dice.
  ...
  - **Vale su tutta la macchina, non solo sulla galleria**: `<sottodominio pannello>` include `snippets/photocarcifo-bloccati-altrove.conf` (un `403` secco, non la pagina: quella parla di fotografie e su un pannello di controllo non c'entra). Senza, il passaggio dal firewall a nginx avrebbe ristretto la portata del ban senza dirlo — un `/banip` permanente avrebbe smesso di coprire il pannello di amministrazione. Conseguenza: quei due file in `/etc/nginx/sites-available/` dipendono dalle variabili definite dalla configurazione di photocarcifo, e ora sono anch'essi nel backup DR.
...
**Siti abilitati** (`/etc/nginx/sites-enabled/`): `000-default-catchall`, `<sottodominio pannello>`, `photocarcifo` (quest'ultimo linkato da `config/nginx/photocarcifo.conf`, secondo la convenzione descritta nel README).
...
**Host**: Synology hostname `magazzino`, indirizzo `<IP NAS Synology>` (secondo `docs/PHOTOCARCIFO.md`; IP non ri-pingato in questa sessione per non generare traffico non necessario, ma il mount attivo lo conferma).
...
**Container**: LXC, identificato come "206 / <nome host container>" nella documentazione interna; hostname del sistema operativo confermato in sessione (`hostname` → `<nome host container>`), IP `<IP container applicazione>`.
...
**NAS**: Synology `<IP NAS Synology>`, montato via NFS come descritto in sezione 19.
...
- **NAS Synology** (`<IP NAS Synology>`): unica fonte dei file media originali; dipendenza critica per la maggior parte delle funzionalità (sezione 19).
...
Repository: `photocarcifo-pwa` (macchina di sviluppo Windows dell'operatore, percorso locale omesso). **Vanilla JavaScript**, nessun framework, nessun bundler, nessuna build step: i file sotto `pwa/` sono serviti staticamente così come sono, montati dal backend FastAPI su `/app` (`app/main.py`: `app.mount("/app", StaticFiles(directory=..., html=True), name="pwa")`).
...
Repository separato: `photocarcifo-mobile` (macchina di sviluppo Windows dell'operatore, non su questo server, percorso locale omesso). Expo (React Native) + TypeScript + Expo Router. Client puro del backend esistente: nessun database proprio, nessuna logica di privacy duplicata — ogni regola (album privati, scaduti, nascosti, password) resta decisa qui, lato server, a ogni richiesta.
...
1. Accesso al NAS Synology (`<IP NAS Synology>`, share `/volume1/photocarcifo`).
...
Rete: IP fisso `<IP container applicazione>`, raggiungibile dal NAS.
...
```
<IP NAS Synology>:/volume1/photocarcifo  /mnt/magazzino     nfs4  ro,_netdev,soft  0  0
<IP NAS Synology>:/volume1/photocarcifo  /mnt/magazzino-rw  nfs4  rw,_netdev,soft  0  0
```
...
```bash
certbot --nginx -d photocarcifo.ch -d <sottodominio pannello>
