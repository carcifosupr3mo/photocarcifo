...
**Canonical / URL lingua**: l'italiano non ha prefisso nell'indirizzo (es. `/n/bmx`); le altre 4 lingue usano un prefisso (`/en/n/bmx`, ecc.), gestito dal middleware `lingua_nell_indirizzo` in `main.py`. Un middleware separato (`barra_finale`) normalizza `/n/bmx/` → `/n/bmx` con redirect 301 per evitare contenuti duplicati agli occhi dei motori di ricerca.

**Metadata / Open Graph**: esiste una route `/social/{media_id}` dedicata a generare l'anteprima ottimizzata (1200×630) per la condivisione su chat/social; i template includono presumibilmente tag Open Graph (non riletti riga per riga in ogni template in questa sessione).

**Structured data**: non verificato nel dettaglio in questa sessione se sono presenti dati strutturati JSON-LD nei template — vedi sezione 37.

**Pagine indicizzabili**: home, album pubblici e visibili, pagine informative (`/chi-sono`, `/privacy`, `/recensioni`, `/radunimoto`, `/novita`).

**Pagine escluse**: pannello admin, album privati/nascosti, pagine di download/zip/miniatura, pagine di ricerca (query string), condivisioni singole (`/f/`, `/fs/`) — coerente sia con `robots.txt` sia con il filtro IndexNow.
...
- **Fail2ban**: attivo sul server, con 8 jail configurate (verificato con `fail2ban-client status`): `nginx-botsearch`, `nginx-limit-req`, `pannello-login`, `photocarcifo-login`, `photocarcifo-scansioni`, `photocarcifo-scansioni-attivita`, `sshd`, `photocarcifo-manuale` (aggiunta 06/09/2026, vedi sotto).
- **Ban manuale da bot Telegram** (`/banip <ip>`, `/unbanip <ip>` in `script/photocarcifo-bot.py`, aggiunto 06/09/2026): non un secondo sistema di blocco, una jail fail2ban in più nello stesso meccanismo delle altre (`photocarcifo-manuale`, definita in `/etc/fail2ban/jail.d/photocarcifo.local`, **non tracciata in git** come le altre 7 — solo configurazione di sistema). Nessun log la alimenta (il filtro `filter.d/photocarcifo-manuale.conf` non può avere corrispondenza per costruzione): l'unico modo di finirci dentro è il comando esplicito. `bantime = -1`: permanente finché non arriva `/unbanip`, a differenza dei ban automatici che scadono da soli.
  ...
  - **Persistenza**: nel database interno di fail2ban stesso (`/var/lib/fail2ban/fail2ban.sqlite3`), non un file/tabella nuovi — verificato con un `systemctl restart fail2ban` reale: il ban di prova è sopravvissuto. Il blocco vive interamente fuori dal processo Python (regola nftables), quindi resta attivo anche ad app o bot fermi.
  ...
  - **Comportamento HTTP invariato**: il blocco è networking (nftables `reject`, azione `iptables-multiport` — stessa di `photocarcifo-login`/`nginx-botsearch`/`nginx-limit-req`), non è mai passato dall'applicazione: nessuna route, nessun codice di stato, nessun middleware toccato.
